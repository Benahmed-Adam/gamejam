...
class Slider:
    def __init__(self, screen: pg.Surface, pos: tuple, width: int, height: int, button_width: int, max_value: int, current_value : int) -> None:
        self.screen = screen
        self.pos = pos
        self.width = width
        self.height = height
...
class Button:
    def __init__(self, game, text: str, size: tuple, font_size: int, pos: tuple) -> None:
        ...
        pg.draw.rect(self.out, (170, 170, 170, 240), [0, 0, self.size[0], self.size[1]], border_radius=20)
        pg.draw.rect(self.on, (100, 100, 100, 240), [0, 0, self.size[0], self.size[1]], border_radius=20)
...
class TextZone:
    def __init__(self, game, default_text: str, size: tuple, font_size: int, pos: tuple, max_length: int, accept_type: list) -> None:
    ...
    def event(self, event):
        if event.type == pg.MOUSEBUTTONDOWN:
            ...
            if pg.Rect(self.pos[0] - self.size[0] // 2, self.pos[1] - self.size[1] // 2, self.size[0], self.size[1]).collidepoint(mouse_pos):
            ...
            if event.key == pg.K_BACKSPACE:
                ...
                if len(self.text) < self.max_length and type_code in self.accept_type:
